...
class Moon {
  ...
  launch = (shipType, angle) => {
    if (this.getStrength() < shipTypes[shipType].cost) return;

    const time = new Date() / 1000;
    this.strength.value = this.getStrength() - shipTypes[shipType].cost;
    this.strength.time = time;
    const moonX = this.getX();
    const moonY = this.getY();
...
class Planet {
  constructor(sun) {
    ...
    for (let i = 0; i < Math.round(Math.random() * 3); i += 1) {
      new Moon(this);
    }
  }
  ...
  launch = (shipType, angle) => {
    ...
    const time = (new Date() / 1000);
    const planetX = this.getX();
    const planetY = this.getY();
...
class Ship {
  ...
  launch(shipType, angle) {
    ...
    const time = (new Date() / 1000);
    const shipX = this.getX();
    const shipY = this.getY();
...
class Sun {
  constructor() {
    ...
    for (let i = 0; i < 2 + Math.round(Math.random() * 10); i += 1) {
  ...
  collisionDetection() {
    ...
    const shipInteraction = (ship, ship2, shipDistance) => {
      ...
      if (
        ...
        ship.intercept = ship2.id;
        const [shipX, shipY] = shipLocations[ship.id];
        const [shipX2, shipY2] = shipLocations[ship2.id];
        ...
        if (sunDistance < sun.size) {
          ...
          Object.values(sun.planets).forEach((planet) => {
            ...
            if (planetDistance < planet.size && ship.planetId !== planet.id) {
              ...
              Object.values(planet.moons).forEach((moon) => {
                const moonAngle = moon.angle.value
                ...
                if (moonDistance < moon.size && ship.moonId !== planet.id) {
                  ...
                  } else if (moonStrength < ship.people) {
                    if (ship.people > 0) {
