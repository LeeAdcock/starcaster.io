      ...
      <div style={{
        ...
        <ButtonGroup>
            ...
            <Dropdown as={ButtonGroup} 
                ...
                onSelect={e=>{setMissile(e); setRadioValue('missile'); props.changeVehicle(e)}}
                >
                <ToggleButton
                    key='missile'
                    id={`radio-missile`}
                ...
                <Dropdown.Menu variant="outline-info">
                    {Object.entries(missileDisplayNames).map(([id, name]) => 
                        <Dropdown.Item active={missileValue===id} eventKey={id}>{name}</Dropdown.Item>
                    )}
                </Dropdown.Menu>
            </Dropdown>
            ...
            <Dropdown as={ButtonGroup} 
                ...
                <Dropdown.Menu variant="outline-info">
                    {Object.entries(planitaryShieldDisplayNames).map(([id, name]) => 
                        <Dropdown.Item active={planitaryShieldValue===id} eventKey={id}>{name}</Dropdown.Item>
                    )}
                </Dropdown.Menu>
            </Dropdown>
            ...
            <Dropdown as={ButtonGroup} 
                ...
                <Dropdown.Menu variant="outline-info">
                    {Object.entries(carrierDisplayNames).map(([id, name]) => 
                        <Dropdown.Item active={carrierValue===id} eventKey={id}>{name}</Dropdown.Item>
                    )}
                </Dropdown.Menu>
            </Dropdown>
